...
def load_procurement_data():
    try:
        file_id = "14INoo53ikFYJ9wBUTSvy2PvcfEHLkbXN"
        output = "Pharma_Procurement_Transactions.csv"
...
@st.cache_resource
def load_bert_model():
    """Load BERT model and components using Google Drive for safetensors and GitHub for other files"""
    try:
        # Google Drive file ID for model.safetensors
        safetensors_id = "1EGv051rti6IL5HyBwMJJeiT6Bp4qM6q1"
        
        # GitHub URLs for other files
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            # Download safetensors from Google Drive
            safetensors_path = os.path.join(temp_dir, 'model.safetensors')
            gdown.download(f"https://drive.google.com/uc?id={safetensors_id}", safetensors_path, quiet=False)
            ...
            # Load label encoder (unchanged from original code)
            label_encoder_url = "https://raw.githubusercontent.com/kheiriddine/Models_Repositories/main/model_fin_xgboost_classifier/label_encoder.json"
            le_response = requests.get(label_encoder_url)
            if le_response.status_code != 200:
                st.error(f"Failed to load label encoder from {label_encoder_url}")
...
def load_distilbert_model():
    """Load DistilBERT model and components using Google Drive for safetensors and GitHub for other files"""
    try:
        # Google Drive file ID for model.safetensors
        safetensors_id = "1hZ8c1Yx4ogo_yk-_FNLDlVkGEEuNteuo"
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            safetensors_path = os.path.join(temp_dir, 'model.safetensors')
            gdown.download(
                f"https://drive.google.com/uc?id={safetensors_id}",
                safetensors_path,
                quiet=False
            )
            
            # Verify safetensors downloaded successfully
            if not os.path.exists(safetensors_path):
                st.error("Failed to download model.safetensors from Google Drive")
                return None, None, None
